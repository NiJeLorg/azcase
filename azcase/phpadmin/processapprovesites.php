...
for ($lt = 0; $lt < pg_numrows($siteidloopresult); $lt++) {
	...
	if (isset($_REQUEST["$siteidloop"])) {
		...
		if ($pgerror1!=FALSE) {
			require('header.php');
			echo "<h3>Save Error</h3><p>An error occured when you attempted to approve a site. An email was sent to NiJeL and we will address this problem. We're sorry for any inconvenience</p>";
			require('footer.php');
			$to = "jd@nijel.org";
		...
		for ($lt2 = 0; $lt2 < pg_numrows($userresult); $lt2++) {
			...
			Hello $email,
			<br /><br />
			Thank you again for adding your site, $sitename, located at $locationname</strong> ($printaddress) to the AZ Afterschool Program Directory! Your recent additions to this site have been approved, and you can now view your site on our map by following the link below and clicking on \"Find Programs Near You.\"
			<br /><br />
			<a href=\"$url\">$url</a>
			<br /><br />
			Thanks,
...
header("Location: http://maps.nijel.org/azcase_dev/azcase/phpadmin/thankyouapprove.php");
?>
