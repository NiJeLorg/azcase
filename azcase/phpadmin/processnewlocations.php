...
if (!$locationname1 || !$address1 || !$city1 || !$state1 || !$zip1) {
	...
	if ($countaddress>0) {
		require('header.php');
		echo "<body><h3>Error: Location Already Exists in the System</h3><p>It appears that you attmepted to add a location that already exists in the system. Please <a href=\"newlocations.php\"go back</a> and search for this location on the map. Thanks!</p>";
		require('footer.php');
		die();
	}else{
		...
		if ($pgerror1!=FALSE) {
			require('header.php');
			echo "<h3>Save Error</h3><p>An error occured when you attempted to submit a new location. An email was sent to the AZ Afterschool Program Directory adminstrator and we will address this problem. We're sorry for any inconvenience</p>";
			require('footer.php');
			$to = "jd@nijel.org";
...
header('Location: http://maps.nijel.org/azcase_dev/azcase/phpadmin/endnewlocations.php');
?>
