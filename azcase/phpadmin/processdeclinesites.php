...
for ($lt = 0; $lt < pg_numrows($siteidloopresult); $lt++) {
	...
	if ($_REQUEST["$siteidloop"]=='t') {
		...
		if ($pgerror1!=FALSE) {
			require('header.php');
			echo "<h3>Save Error</h3><p>An error occured when you attempted to decline a site. An email was sent to NiJeL and we will address this problem. We're sorry for any inconvenience</p>";
			require('footer.php');
			$to = "jd@nijel.org";
		...
		for ($lt2 = 0; $lt2 < pg_numrows($userresult); $lt2++) {
			...
			if (!$comments) {
				...
				$comments = '<strong>Specific comments from Arizona Center for Afterschool Excellence:</strong><br />' . $comments . '<br /><br />';
			}

			$message = "
			Hello $email,
			<br /><br />
			Thank you again for adding your site, $sitename, located at $locationname</strong> ($printaddress) to the AZ Afterschool Program Directory. Unfortunately, we declined to include this site in our directory. The data you added does not meet our criteria for inclusion on the map.
			<br /><br />
			$comments
...
header("Location: https://azcase.nijel.org/php/thankyoudecline.php");
?>
