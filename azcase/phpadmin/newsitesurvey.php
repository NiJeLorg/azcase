...
// increment site id
if (!$lastsiteid) {
	$siteidquery = "SELECT siteid FROM azcase_user_sites_junction WHERE userid = $userid AND newsite = TRUE ORDER BY siteid LIMIT 1;";
...
if ($countsite==0) {
	header("Location: http://maps.nijel.org/azcase_dev/azcase/phpadmin/summersites.php");
}else{
...
?>
<body>
<h3 class="azcase-text-color">Add Additional Information for <?php echo $sitename; ?></h3>
<p>Please complete this form below for the site, <strong><?php echo $sitename; ?></strong>, located at <strong><?php echo $locationname; echo ' (' . $printaddress . ')'; ?></strong>. Please note that these forms are <em>optional</em>, but the data you enter below will greatly assist the Arizona Center for Afterschool Excellence in our advocacy work. Thanks!</p>
<?php

// add in the new locations form
require('addnewsitesurveyform.php'); 
